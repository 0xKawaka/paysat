...
const SECURE_STORE_FALLBACK_PREFIX = "starknet.secure.fallback.";

const ensureSecureStoreAvailable = async (): Promise<boolean> => {
  try {
...
const setSecureItem = async (key: string, value: string): Promise<void> => {
  const isAvailable = await ensureSecureStoreAvailable();
  if (isAvailable) {
    await SecureStore.setItemAsync(key, value);
    return;
  }

  await AsyncStorage.setItem(`${SECURE_STORE_FALLBACK_PREFIX}${key}`, value);
};
...
const getSecureItem = async (key: string): Promise<string | null> => {
  ...
  return await AsyncStorage.getItem(
    `${SECURE_STORE_FALLBACK_PREFIX}${key}`,
  );
};
...
const deleteSecureItem = async (key: string): Promise<void> => {
  ...
  await AsyncStorage.removeItem(`${SECURE_STORE_FALLBACK_PREFIX}${key}`);
};
...
export const StarknetConnectorProvider: React.FC<{
  ...
  const randomHex = (length: number): string => {
    const size = Math.max(0, Math.ceil(length / 2));
    ...
    if (cryptoObj?.getRandomValues) {
      ...
      // Fallback for environments without a native crypto implementation.
      for (let i = 0; i < randomBytes.length; i += 1) {
        randomBytes[i] = Math.floor(Math.random() * 256);
      }
      if (__DEV__) {
        console.warn(
          "Secure random generator unavailable; falling back to Math.random().",
        );
      }
    }
  ...
  const storePrivateKey = async (
    ...
    await setSecureItem(key, privateKey).catch((error) => {
  ...
  const clearPrivateKeys = async (appName: string): Promise<void> => {
    try {
      ...
      if (keys) {
        ...
        await Promise.all(
          parsedKeys.map((key) => deleteSecureItem(key)),
        );
  ...
  const getPrivateKey = async (key: string): Promise<string | null> => {
    try {
      const privateKey = await getSecureItem(key);
      if (privateKey) {
        return privateKey;
      } else {
  ...
  const disconnectAndDeleteAccount = useCallback(async () => {
    ...
    await clearPrivateKey(activeStorageKey);
    disconnectAccount();
  }, [account, activeStorageKey, clearPrivateKey, disconnectAccount]);
